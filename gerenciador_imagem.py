...
def gerar_imagem_huggingface(prompt):
    ...
    if not token:
        ...
        raise RuntimeError(
            "HF_TOKEN não foi encontrado nos "
    ...
    try:
        ...
        raise RuntimeError(
            "A biblioteca huggingface_hub não está "
            "instalada. Adicione huggingface_hub ao "
            "requirements.txt."
        )
    ...
    try:
        ...
        raise RuntimeError(
            "Erro na geração de imagem pelo "
            f"Hugging Face: {erro}"
        )
...
def gerar_imagem(prompt):
    ...
    try:
        ...
        caminho = gerar_imagem_huggingface(
            prompt
        )
