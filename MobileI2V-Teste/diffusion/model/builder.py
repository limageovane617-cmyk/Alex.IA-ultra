...
# CORRIGIDO:
# Antes: from mmcv import Registry
...
def get_vae(
    ...
    if name == "sdxl" or name == "sd3":
        ...
        print(
            colored(
                f"[DC-AE] Loading model from {model_path}",
                attrs=["bold"]
            )
        )
        ...
        print(
            colored(
                f"[AutoencoderDC] Loading model from {model_path}",
                attrs=["bold"]
            )
        )
        ...
        print(
            colored(
                f"[Video-vae] Loading model from {model_path}",
                attrs=["bold"]
            )
        )
...
def vae_encode(
    ...
    if name == "sdxl" or name == "sd3":
        ...
        scaling_factor = (
            ae.cfg.scaling_factor
            if ae.cfg.scaling_factor
            else 0.41407
        )
...
def vae_decode(
    ...
    if name == "sdxl" or name == "sd3":
        ...
        vae_scale_factor = (
            2 ** (
                len(
                    ae.config.encoder_block_out_channels
                ) - 1
            )
            if hasattr(ae, "config")
            and ae.config is not None
            else 32
        )
        ...
        try:
            ...
            print(
                "Warning: Ran out of memory when "
                "regular VAE decoding, retrying "
